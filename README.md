...
Memory Agent 是一个基于 LangGraph、Chainlit、Qdrant 和本地 embedding 模型构建的长期记忆聊天代理。
...
├── docker-compose.yml           # Docker Compose 运行配置，包含 Qdrant 服务
├── Dockerfile                   # 应用镜像
├── main.py                      # CLI 演示入口
├── memory_agent/
...
Docker Compose 会同时启动 Memory Agent 和 Qdrant 服务，是推荐的快速启动方式。
...
GPU 支持是可选项。可加速本地 embedding 推理，但需要 NVIDIA GPU、可用的主机驱动以及 Docker 的 NVIDIA Container Toolkit。Compose 的 GPU 预留格式遵循 Docker 官方指南：
...
GPU 覆盖配置会切换为 CUDA 12.6 的 PyTorch wheel，并请求 Compose 预留 NVIDIA GPU 设备。推荐保留 `EMBEDDING_DEVICE=auto` 以便可用时自动启用 CUDA，或显式指定：
...
- Qdrant 与 embedding 相关工作在异步友好的后台线程执行，减少 Chainlit 主流程阻塞概率。
